...
> OpenSpec 实战教程：每个命令怎么用、什么时候用、完整示例（以 Java AI 多模块课程项目为例）。  
> 安装说明见 [SPEC-DRIVEN-DEVELOPMENT.md](./SPEC-DRIVEN-DEVELOPMENT.md)；项目架构见 [PROJECT-ARCHITECTURE.md](./PROJECT-ARCHITECTURE.md)。
...
| 传统方式 | OpenSpec 方式 |
|----------|---------------|
| 「帮我在 jc-rag-kb 里加个功能」→ AI 直接改代码 | 先对齐做什么、为什么、怎么做 → 审阅文档 → 再按 tasks 写代码 → 归档 |
...
不要在子模块目录（如 `spring-ai-alibaba/`）打开。
...
│       └── archive/         ← 已归档的历史变更
│           └── 2026-06-11-add-project-context-and-diagrams/
...
```
/opsx-explore 梳理 jc-rag-kb 的 RAG 查询完整链路
```

```
/opsx-explore 我想给销售 Agent 加导出 Excel，但不确定改哪个模块
```
...
```
你：/opsx-explore jc-rag-kb 文档上传后是怎么索引的？

AI：上传 → MinIO → 异步分块 → Embedding → pgvector ...

你：好，帮我 propose
你：/opsx-propose 为 jc-rag-kb 添加 PDF 分页索引策略
```
...
```
/opsx-propose 为 jc-rag-kb 的 RagQueryController 增加按 metadata 过滤
```
...
```
proposal 里把范围缩小，只改 jc-rag-kb，不要动 spring-ai-alibaba
```
...
- 确认改在正确子项目（RAG 产品改 `jc-rag-kb`，不是 `spring-ai-alibaba`）
- 本地验证：`cd jc-rag-kb && mvn compile`
...
### 场景：给 jc-rag-kb 增加「按文档类型过滤查询」

**Step 1 — 探索（可选）**

```
/opsx-explore jc-rag-kb 查询时 metadata 存在哪、怎么过滤
```

**Step 2 — 提出变更**

```
/opsx-propose 为 jc-rag-kb 增加按文档类型 metadata 过滤 RAG 查询
```
...
项目内已完成一次完整 OpenSpec 闭环，可作为参考：

| 步骤 | 命令 | 结果 |
|------|------|------|
| 提出 | `/opsx-propose 基于现有代码生成项目背景到 config.yaml 并生成流程图` | 变更 `add-project-context-and-diagrams` |
| 实现 | `/opsx-apply` | 13/13 tasks 完成 |
| 同步 | `/opsx-sync` | `openspec/specs/project-context/spec.md` |
| 归档 | `/opsx-archive add-project-context-and-diagrams` | 移至 `archive/2026-06-11-add-project-context-and-diagrams/` |

**交付物**

- `openspec/config.yaml` — 14 个子项目、端口、课程映射、编码约定
- `docs/PROJECT-ARCHITECTURE.md` — 8 张 Mermaid 架构/时序图

**归档后查看历史**

```
openspec/changes/archive/2026-06-11-add-project-context-and-diagrams/
├── proposal.md
├── design.md
├── tasks.md
...
propose 时明确范围，并确保 `openspec/config.yaml` 有项目背景：

```
只改 jc-rag-kb，不要动 spring-ai-alibaba
```
...
AI 每次 propose/apply 都会读。应包含：子项目清单、端口冲突、框架选型、编码约定。新增子项目后记得更新。
...
- [项目架构与流程图](./PROJECT-ARCHITECTURE.md)
...
| 日期 | 说明 |
|------|------|
| 2026-06-11 | 初版：五命令详解、实战案例、完整生命周期 |
