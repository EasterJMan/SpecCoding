...
- [项目还没有 OpenSpec 时（首次接入）](#项目还没有-openspec-时首次接入)
- [使用前准备（已初始化项目）](#使用前准备已初始化项目)
- [文件结构说明](#文件结构说明)
- [完整生命周期](#完整生命周期)
- [五个命令详解](#五个命令详解)
...
## 项目还没有 OpenSpec 时（首次接入）

很多项目是**先用 Cursor 写代码，后来才听说 OpenSpec**。此时项目里还没有 `openspec/` 目录，输入 `/` 也看不到 `/opsx-propose` 等命令——**这是正常的**，需要先完成「全局安装 + 项目初始化」两步。

### 怎么判断项目还没接入？

满足以下**任意一条**，说明尚未在本项目初始化 OpenSpec：

| 检查项 | 未初始化时的表现 |
|--------|------------------|
| 目录 | 项目根目录**没有** `openspec/` 文件夹 |
| 配置 | **没有** `openspec/config.yaml` |
| Cursor | 输入 `/` **看不到** `/opsx-propose`、`/opsx-explore` 等 |
| CLI | 在项目根执行 `openspec list` 报错或提示未初始化 |

> OpenSpec 分两层：**本机全局 CLI**（装一次到处用）和**每个项目内的初始化**（每个仓库各做一次）。

### 第一步：本机安装 OpenSpec CLI（只需做一次）
...
若 `openspec: command not found`，确认 npm 全局 bin 目录在 PATH 中。

### 第二步：在项目里初始化（每个仓库做一次）

在终端进入**项目根目录**（含 `.git` 或顶层 `pom.xml` / `package.json` 的那一层）：

```bash
cd <项目根目录>

# 非交互式初始化，并配置 Cursor 集成
...
|------|------|
| `.` | 在当前目录初始化（不新建子目录） |
| `--tools cursor` | 生成 Cursor 的 Slash 命令与 Skills |
| `--force` | 非空目录时跳过确认（已有文件的项目常用） |
...
your-project/
├── openspec/
│   ├── config.yaml       ← 项目配置（建议随后补充 context）
│   ├── changes/          ← 之后 propose 产生的变更放这里
│   └── specs/            ← archive/sync 后的正式规格
└── .cursor/
    ├── commands/opsx-*.md    ← /opsx-propose 等命令
    └── skills/openspec-*     ← Agent Skills
```

### 第三步：重启 Cursor

`openspec init` 写入 `.cursor/commands/` 后，**必须完全退出并重新打开 Cursor**，Slash 命令才会出现。

### 第四步（强烈建议）：填写项目背景

编辑 `openspec/config.yaml`，在 `context:` 下写清技术栈、模块划分、约定。AI 在 propose/apply 时会自动读取，减少改错模块。
...
context: |
  ...
  仓库结构：多模块 Maven，各模块独立启动
  约定：controller → service → repository
```

也可在 Agent 里说：

```
请根据当前代码库，帮我完善 openspec/config.yaml 的 context 块
```
...
└───────────────────────────┬─────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────┐
│  C. 重启 Cursor → 出现 /opsx-* 命令                       │
└───────────────────────────┬─────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────┐
│  D. 可选：编辑 openspec/config.yaml 补充 context         │
└───────────────────────────┬─────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────┐
│  E. 日常使用：explore → propose → apply → sync → archive │
└─────────────────────────────────────────────────────────┘
```

### 已有项目（存量 / brownfield）注意事项

- **不要在子模块目录 init**：应在 monorepo **根目录**执行，否则 AI 读不到全局结构。
- **与 Spec-Kit 共存**：若项目里还有 `.specify/`（Spec-Kit），两者可并存；`.cursor/` 下会同时有 `opsx-*` 与 `speckit-*`，日常选一套主流程即可。
- **升级 OpenSpec 后**：在本机 `npm install -g @fission-ai/openspec@latest`，再到项目里执行 `openspec update` 刷新 Cursor 集成。

更详细的安装与 Spec-Kit 对比见 [SPEC-DRIVEN-DEVELOPMENT.md](./SPEC-DRIVEN-DEVELOPMENT.md)。

---

## 使用前准备（已初始化项目）

以下前提：**项目已完成** [首次接入](#项目还没有-openspec-时首次接入)（存在 `openspec/` 且 Cursor 里能看到 `/opsx-*`）。
...
1. **项目是否已 init？** 根目录是否有 `openspec/` — 没有则见 [首次接入](#项目还没有-openspec-时首次接入)
2. **是否打开项目根目录？** 不要在子模块里打开 Cursor
3. **是否重启过 Cursor？** init 或 `openspec update` 后必须重启
4. **本机是否装了 CLI？** 终端执行 `openspec --version`
5. **刷新集成：** 在项目根执行 `openspec update`
...
| 2026-06-11 | 新增：项目尚未 init OpenSpec 时的首次接入说明 |
